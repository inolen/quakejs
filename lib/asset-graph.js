...
AssetGraph.prototype.addMap = function (name, game, buffer) {
	...
	this.getVertices().filter(function (assetV) {
		return assetV.data.type !== ASSET.MAP && assetV.data.game === game && assetV !== mapV && assetV.id.indexOf(mapName + '.') !== -1;
...
AssetGraph.prototype.getMapAssets = function (game, map) {
	var self = this;
	var verts = [];
	var checked = {};
...
AssetGraph.prototype.getCommonAssets = function (game, isWhitelisted) {
	var self = this;
	var verts = [];
	var checked = {};
	...
	this.getVertices().filter(function (v) {
		...
		if (references.length < self.referenceThreshold && !isWhitelisted(v.id)) {
			return;
		}
